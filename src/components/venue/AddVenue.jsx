...
export function AddVenue({ setCreateVenue }) {
  const token = JSON.parse(getStorage('user')).accessToken;
  const endpoint = `${constants.base}${constants.holidaze.base}${constants.holidaze.venues.clear}`;
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    postData(endpoint, venueData, token);
    setSearching(true);
  };
  ...
  return (
    <div className="w-full max-w-lg bg-white p-6 rounded-lg shadow-md relative">
      ...
      {!searching && (
        <>
          <h2 className="text-2xl font-semibold mb-4 text-center">Add Venue</h2>
